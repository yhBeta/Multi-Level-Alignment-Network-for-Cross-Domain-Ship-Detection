...
class SSDDDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, transforms=None, is_source= False):
        ...
        # self.ids = [x.split('.')[0] for x in os.listdir(os.path.join(self.root, "JPEGImages"))]
        self.ids = []
        for x in os.listdir(os.path.join(self.root, "JPEGImages")):
            # if self.image_set == 'train':
            #     if x.split('.')[0][-1] != '1' and x.split('.')[0][-1] != '9':
            #         self.ids.append(x.split('.')[0])
            # else:
            #     if x.split('.')[0][-1] == '1' or x.split('.')[0][-1] == '9':
            #         self.ids.append(x.split('.')[0])
            self.ids.append(x.split('.')[0])
        ...
        #             self.ids.append(x.split('.')[0])
        #     else:
        #         if int(x.split('.')[0]) > 500:
        #             self.ids.append(x.split('.')[0])

        cls = SSDDDataset.CLASSES
    ...
    def map_class_id_to_class_name(self, class_id):
        return SSDDDataset.CLASSES[class_id]
